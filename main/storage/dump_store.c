...
// 返回 0 表示 NVS 失败, 调用方必须放弃保存以避免文件名冲突
...
esp_err_t dump_store_init(void) {
    ...
    ESP_LOGI(TAG, "littlefs mounted, used=%u/%u", (unsigned)used, (unsigned)total);
    return ESP_OK;
}
...
static void merge_mfc_into(mfc_dump_t* base, const mfc_dump_t* upd) {
    ...
    for (uint16_t b = 0; b < upd->block_count && b < MFC_MAX_BLOCKS; b++) {
        if (upd->block_read[b]) {
...
esp_err_t dump_store_save_mfc(const mfc_dump_t* d, const char* name, int64_t created_ms, char out_id[40]) {
    // 同 UID 已有 dump 则先合并再覆盖,避免重复扫卡产生多条碎片记录
    char old_id[40]   = {0};
    char old_name[64] = {0};
    ...
    if (has_old) {
        ...
        if (dump_store_read_bin(old_id, &old_bin, &old_len) == ESP_OK) {
            ...
            if (merged) {
                ...
                if (old_pages > NTAG_MAX_PAGES) old_pages = NTAG_MAX_PAGES;
                ...
                for (uint16_t p = 0; p < d->total_pages && p < NTAG_MAX_PAGES; p++) {
                    if (d->page_read[p]) {
...
void dump_store_recover_trailer_keys(mfc_dump_t* dump) {
    // 全零 key 合法,不能靠 key 判 trailer 是否读过; access bits 未读时必为 0,以此区分
...
esp_err_t dump_store_load_mfc(const char* id, mfc_dump_t* dump) {
    ...
    bool is_4k = (m.block_or_page_count >= MFC_4K_BLOCKS) || (bin_len >= MFC_4K_BLOCKS * MFC_BLOCK_LEN);
    ...
    dump->block_count  = is_4k ? MFC_4K_BLOCKS : MFC_1K_BLOCKS;
