...
// NTAG21x 命令码 (NXP NT2H1311 datasheet)
...
ntag_type_t ntag_detect_type(void) {
    ...
    // GET_VERSION 回包 byte 6 是 storage size code (见 datasheet 表 "Storage size")
    switch (rx[6]) {
...
static uint16_t pages_for(ntag_type_t t) {
    switch (t) {
        case NTAG_213: return 45;
        case NTAG_215: return 135;
        case NTAG_216: return 231;
        default:       return 16;  // Ultralight 兜底
    }
}
...
esp_err_t ntag_full_write(const ntag_dump_t* dump) {
    // page 0/1 (UID) 只读, page 2 (lock), page 3 (CC) 改动会锁卡, 这里只写用户区
    int consecutive_fail = 0;
