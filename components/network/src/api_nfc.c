/**
 * @file api_nfc.c
 * @brief NFC 操作相关 API：scan / read / write / ndef
 *
 * 长操作（read / write）走后台 task；进度通过 WebSocket 推送。
 * 全局只允许一个 NFC 任务同时运行；使用 binary semaphore 保护——
 * 由 HTTP 任务 take、后台任务 give，不符合 mutex 所有权协议，故用 binary。
 */
...
typedef struct {
    char        task_id[16];
    char        name[64];       // 用户给的备注
    int64_t     created_ms;     // 浏览器 Unix 毫秒（设备无 RTC）
} read_task_arg_t;
...
static void task_full_read(void* arg) {
    ...
    if (tgt.type == PN532_CARD_MIFARE_CLASSIC_1K || tgt.type == PN532_CARD_MIFARE_CLASSIC_4K) {
        ...
        char id[40];
        dump_store_save_mfc(dump, a->name, a->created_ms, id);
        ...
        char id[40];
        dump_store_save_ntag(dump, &tgt, a->name, a->created_ms, id);
        char result[96];
...
static esp_err_t handle_read(httpd_req_t* req) {
    ...
    // 先复制 task_id，避免 task 启动后 a 被释放导致竞态
    char resp[64];
...
static esp_err_t handle_ndef_read(httpd_req_t* req) {
    ...
    ntag_dump_t* dump = calloc(1, sizeof(ntag_dump_t));
    if (!dump) return send_json(req, "{\"ok\":false,\"err\":\"内存不足\"}");
...
static esp_err_t handle_ndef_write(httpd_req_t* req) {
    ...
    cJSON* j = cJSON_Parse(body);
    if (!j) return send_json(req, "{\"ok\":false,\"err\":\"JSON 格式错误\"}");
